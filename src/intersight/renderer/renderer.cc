...
void Renderer::render_mesh(const Mesh3d<T> &mesh, const Camera<T> &camera,
  ...
  for (const auto &face : mesh.get_faces()) {
    Vector3d<T> point_1 = vertices[face.get_vertex_indices()[0]];
    Vector3d<T> point_2 = vertices[face.get_vertex_indices()[1]];
    Vector3d<T> point_3 = vertices[face.get_vertex_indices()[2]];
