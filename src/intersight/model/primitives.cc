...
template <typename T> Vector3d<T> Vector3d<T>::operator+(const T value) const {
  return Vector3d(x_ + value, y_ + value, z_ + value);
}

template <typename T> void Vector3d<T>::operator+=(const Vector3d<T> &vector) {
  x_ += vector.x_;
  y_ += vector.y_;
  z_ += vector.z_;
}

template <typename T> void Vector3d<T>::operator+=(const T value) {
  x_ += value;
  y_ += value;
  z_ += value;
}
...
template <typename T> void Vector3d<T>::operator/=(const T scalar) {
  x_ /= scalar;
  y_ /= scalar;
  z_ /= scalar;
}
